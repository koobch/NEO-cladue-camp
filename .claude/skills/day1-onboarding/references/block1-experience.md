...
|---------------|----------------|
| 슬랙/메일/캘린더 하나씩 열어서 30분 | 한 문장으로 자동 수집 + 브리핑 생성 |
| "보고서 만들어줘" → 뭘 넣을지 모름 | 모호한 요청 → Claude가 질문으로 명확화 |
...
강사가 Git Bash(또는 VS Code 터미널)에서 아래를 입력하고, Claude가 Slack 메시지 수집 → git log 확인 → 문서 생성까지 자동으로 진행하는 과정을 보여준다:

```
/weekly-sync
```

핵심 포인트: **한 마디면 끝**. Slack, git, 문서 작성이 하나의 Skill로 자동화된다. 7일 후에는 여러분도 이런 걸 직접 만들 수 있다. 예를 들어 `/경영회의-브리핑`이라고 치면 각 팀 주간 이슈가 자동으로 수집되는 것이다.
...
```
경영회의 자료 준비해줘. 모호한 부분은 AskUserQuestion으로 질문해서 명확하게 만들어
```

Claude가 "어떤 회의인가요? (주간/월간/긴급)", "참석 대상은? (C-level/본부장/팀장급)", "필수 포함 항목은? (재무현황/인력현황/이슈사항)" 등 **선택지를 제시하며 질문**하는 과정을 체험시킨다.
...
2. **데모 2** 실행: 일부러 모호하게 요청 → 그다음 AskUserQuestion 활용 버전으로 재요청 → 차이 체험. 경영지원 업무로 해보자 (예: "예산 현황 정리해줘", "인사 이슈 요약해줘")
