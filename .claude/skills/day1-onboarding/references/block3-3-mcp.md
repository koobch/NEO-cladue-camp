...
| 비유 | 외부 도구 플러그 — Slack, Calendar, Gmail, Google Drive 등을 Claude에 연결하는 오픈 표준 프로토콜 |
| 예시 | "Slack 메시지 읽어줘", "캘린더 일정 확인해줘", "드라이브에서 파일 찾아줘" |

```
Claude ──── "이번 주 경영회의 일정 확인해줘"
  │
  ▼ 툴 콜링
┌──────────┐    MCP 프로토콜    ┌──────────┐
│ Claude   │ ◀═══════════════▶ │ Google   │
...
```
MCP가 뭔지 쉽게 설명해줘. 경영지원 업무에서 쓸 수 있는 MCP 서버 예시 3개도 알려줘
```
...
정답: 2번 ("Claude와 외부 도구를 연결하는 표준 프로토콜").
