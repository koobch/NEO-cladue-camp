...
아래 내용을 `%USERPROFILE%\.claude\settings.json` (Git Bash: `~/.claude/settings.json`)에 추가한다:
...
>
> ⚠️ **Windows 참고**: Agent Teams의 분할 창(tmux) 모드는 Windows Terminal, VS Code 터미널에서 지원되지 않는다.
> 기본 In-process 모드는 정상 동작하니 걱정하지 않아도 된다.

### 라이브 데모

> 강사가 Agent Teams를 실행하여 여러 에이전트가 동시에 동작하는 모습을 보여준다.
> "지금 보시는 것처럼 각 Agent가 독립된 프로세스에서 동시에 작업하고, 서로 메시지를 주고받습니다."
...
```
✅ 체크리스트
- [ ] settings.json에 CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS 가 "1"로 설정됨
```
