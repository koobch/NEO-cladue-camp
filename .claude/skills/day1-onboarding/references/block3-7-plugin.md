...
| 예시 | "경영지원본부 플러그인" 하나로 Skill + MCP + Hook + Agent 설치 |

```
개별 설치 (Plugin 없이)          Plugin (한 번에)
┌─────────┐                    ┌──────────────────┐
│ Skill A │ ← 수동 복사         │ admin-support    │
...
워크플로우를 강화하는 인기 플러그인이다. 마켓플레이스를 먼저 등록하고, 거기서 설치한다:
...
정답: 2번 ("Skill + MCP + Hook + Agent 등을 하나의 패키지로").
