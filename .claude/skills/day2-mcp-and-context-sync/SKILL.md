...
| 8 | Official Plugin (`/plugin`) | Google Drive | ★★★☆ | ~15min |
...
Block 8:  [Slack] [Notion] [Google Drive] [STUB] [STUB흐름] [STUB포맷]
Block 9:  [Slack] [Notion] [Google Drive] [Google Cal/Gmail] [STUB흐름] [STUB포맷]
Block 10: [Slack] [Notion] [Google Drive] [Google Cal/Gmail] [완성흐름] [완성포맷] → 실행!
```
...
| 8 | 소스 3: Google Drive | `/plugin install` | 소스 3 STUB → 실제 내용 |
| 9 | 소스 4: Google Calendar/Gmail | 커뮤니티 Plugin | 소스 4 STUB → 실제 내용 |
...
- **Block 8 (Plugin → Google Drive)**: Phase A에서 Plugin 개념 설명 + `/plugin install google-drive` + MCP 자동 등록 확인 + 테스트 + 스킬 소스 3 채우기 → Stop. Phase B에서 퀴즈.
- **Block 9 (커뮤니티 Plugin → Google Calendar/Gmail)**: Phase A에서 Plugin 구조 분석 + AskUserQuestion으로 Calendar/Gmail 선택 + Explore로 Plugin 구조 탐색 + 설치 + 스킬 소스 4 채우기 → Stop. Phase B에서 퀴즈.
...
| Block 8 | Google Drive 사용 여부 확인 (미사용 시 대안 Plugin 안내) |
...
Block 8의 Phase A는 **AskUserQuestion을 사용**한다. Google Drive 사용 여부를 확인해야 한다.

Google Drive 사용 시:
1. `/plugin install google-drive` 실행
2. `/mcp`로 local 섹션에 Google Drive MCP 자동 등록 확인
3. 연결 테스트 후 스킬 소스 3 채우기

Google Drive 미사용 시 (Plan B):
...
| Block 8 | `references/block8-plugin-google-drive.md` | `/plugin install`로 Google Drive 연결 + 스킬 소스 3 채우기 |
| Block 9 | `references/block9-skill-google.md` | Plugin 구조 분석 + Google Calendar/Gmail 연결 + 스킬 소스 4 채우기 |
...
| 파일 | 용도 |
|------|------|
| `templates/context-sync.md` | Context Sync 스킬 기본 템플릿 (Slack, Notion, Google Drive, Google Calendar/Gmail 4종 포함) |
...
| 8 | Plugin (`/plugin`) | Google Drive | 공식 Plugin으로 Google Drive 연결 |
| 9 | 커뮤니티 Plugin | Google Calendar/Gmail | Plugin 구조 분석 + Google 연결 |
