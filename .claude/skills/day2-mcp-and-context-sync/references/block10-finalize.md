...
Block 8:  Google Drive ← Official Plugin (/plugin)         ★★★☆
Block 9:  Google Cal/Gmail ← 커뮤니티 Plugin (구조 분석)    ★★★★
```
...
**직원 4명인 카페 (병렬 수집)**
- 한 명은 Slack, 한 명은 Notion, 한 명은 Google Drive, 한 명은 Google Calendar/Gmail
- 각자 맡은 일만 집중 → 4팀 처리에 10분

```
❌ 순차 수집 (직원 1명): 120초
  Slack(30초) → Notion(30초) → Google Drive(30초) → Google Cal/Gmail(30초)

✅ 병렬 수집 (직원 4명): 30초
  Slack(30초)         ─┐
  Notion(30초)         ├── 동시에!
  Google Drive(30초)   │
  Google Cal/Gmail(30초) ─┘
```
...
| **+ Slack 메시지** | #경영회의 등 특정 채널에 요약 발송 | Slack MCP 연결 |
| **+ Notion 페이지** | 경영 대시보드 DB에 새 페이지 생성 | Notion MCP 연결 |
...
AskUserQuestion({
  "questions": [{
    ...
    "options": [
      ...
      {"label": "Markdown + Slack 전송", "description": "파일 저장 + #경영회의 등 Slack 채널에 요약 발송"},
      {"label": "Markdown + Notion 페이지", "description": "파일 저장 + Notion 경영 대시보드에 페이지 생성"}
    ],
    "multiSelect": false
  }]
})
...
Agent(description="Google Drive 수집", prompt="...", subagent_type="general-purpose")
Agent(description="Google Cal/Gmail 수집", prompt="...", subagent_type="general-purpose")
...
수집 결과:
  ...
  Notion: 15개 태스크 ✅
  Google Drive: 8개 문서 ✅
  Google Calendar/Gmail: 5개 일정, 12개 이메일 ✅

하이라이트 3건:
  1. [Slack] #경영회의: 분기 목표 확정
  2. [Google Drive] 성과 보고서 댓글 3건
  3. [Calendar] 내일 10시 팀장회의
...
Notion 포함 시:
```
Notion MCP로 경영 대시보드 데이터베이스에 새 페이지 생성
```
...
[Slack 수집] 진행 중...
[Notion 수집] 진행 중...
[Google Drive 수집] 진행 중...
[Google Cal/Gmail 수집] 진행 중...
```
...
수집 결과:
  ...
  Notion: ✅ 성공 (15개 태스크)
  Google Drive: ❌ 실패 (원인: 인증 토큰 만료)
  Google Cal/Gmail: ✅ 성공 (5개 일정)
...
활용 팁:
  1. 매일 아침 실행하면 "모닝 브리핑"처럼 사용할 수 있습니다
     - 경영회의 전 #경영회의 채널 + 의사결정 기록 + 공유 문서를 한 번에 정리
...
정답: 2번 (Markdown 파일). 별도 설정 없이 바로 사용할 수 있어서 가장 간단하다. 그래서 기본 출력으로 항상 포함된다.
