...
3. 캘린더 열어서 오늘 회의 일정 확인
4. Notion 열어서 경영 대시보드 확인
5. Google Drive 열어서 공유 문서 확인
...
| **8** | Official Plugin (`/plugin`) | **Google Drive** | Plugin 설치 = MCP 자동 등록 |
| **9** | 커뮤니티 Plugin (구조 분석) | **Google Calendar/Gmail** | Plugin의 내부 구조까지 이해 |
...
Block 8:         [Slack] [Notion] [Google Drive] [STUB] ← 소스 3 채움
Block 9:         [Slack] [Notion] [Google Drive] [Google Cal/Gmail] ← 소스 4 채움
...
구조:
  ...
  📌 소스 3: Google Drive  [STUB - Block 8에서 /plugin install로 연결]
  📌 소스 4: Google Cal/Gmail [STUB - Block 9에서 커뮤니티 Plugin으로 연결]
...
AskUserQuestion({
  "questions": [{
    ...
    "options": [
      ...
      {"label": "여러 도구에서 정보를 모아 하나의 문서로 정리한다", "description": "Slack, Notion, Google Drive, Google Calendar/Gmail에서 수집 → 통합"}
    ],
    "multiSelect": false
  }]
})
...
정답: 3번. Context Sync 스킬은 **여러 도구에서 정보를 모아 하나의 문서로 정리**하는 스킬이다. 오늘은 4개 도구를 각각 다른 방법으로 연결하면서 이 스킬을 점진적으로 완성한다.
