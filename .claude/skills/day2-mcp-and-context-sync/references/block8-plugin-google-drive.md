# Block 8: Official Plugin → Google Drive
...
Google Drive는 경영지원 업무에서 공유 문서, 보고서, 회의록 등을 관리하는 핵심 도구다. 공식 Plugin이 제공되며, 설치하면 드라이브 내 파일 검색/읽기/관리가 가능해진다.

### Plan B: Google Drive가 없다면?

Google Drive를 사용하지 않는다면, 다른 공식 Plugin으로 같은 체험을 할 수 있다:
...
> ⚠️ 보안 안내: Plugin 설치 시 인증이 필요할 수 있다. 회사 Google Workspace 계정이 제한될 경우 개인 Google 계정을 사용한다.

## EXECUTE

### Step 1: Google Drive 사용 여부 확인

```json
AskUserQuestion({
  "questions": [{
    "question": "Google Drive를 사용하고 계신가요?",
    "header": "Google Drive 확인",
    "options": [
      {"label": "네, Google Drive 사용합니다", "description": "Google Drive Plugin을 설치합니다"},
...
> **Google Drive가 없어도 괜찮습니다.** 이 블록의 핵심은 "Plugin을 설치하면 MCP가 자동으로 등록된다"는 체험입니다. 다른 공식 Plugin으로도 같은 체험이 가능합니다.
...
→ 설치 가능한 플러그인 목록이 표시됨
→ Google Drive (또는 선택한 다른 Plugin)을 찾는다
```

### Step 3: Plugin 설치

```bash
/plugin install google-drive
```

설치가 완료되면 아래가 자동으로 처리된다:
- Plugin의 `.mcp.json`이 자동 활성화되어 Google Drive MCP 서버가 연결됨
- Google Drive 관련 스킬이 설치될 수 있음
- 필요한 설정이 자동으로 구성됨
...
```
Claude에게 요청: "Google Drive에서 최근 공유된 문서 보여줘"

기대 결과:
  - Google Drive의 파일/문서 목록이 출력됨
  - 경영회의 자료, 보고서, 공유 문서 등이 보임
```

> Google 인증 화면이 나오면 로그인하고 "허용"을 클릭한다.

### Step 6: 스킬 소스 3 채우기

`.claude/skills/my-context-sync/SKILL.md`의 **소스 3: Google Drive** 섹션을 채운다.
...
| 항목 | 값 |
|------|-----|
| 연결 방법 | Official Plugin (`/plugin install google-drive`) |
| MCP 도구 | Google Drive Plugin이 등록한 MCP 도구 |
| 수집 범위 | 최근 7일 업데이트된 공유 문서 |

수집 방법:
Google Drive Plugin의 MCP 도구를 사용하여 문서를 조회한다.
