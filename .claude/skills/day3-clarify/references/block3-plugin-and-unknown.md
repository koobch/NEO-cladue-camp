...
| 분면 | 경영지원 예시 |
|------|-------------|
| **KK** | 매월 작성하는 경영보고서 양식, 급여 지급 프로세스 |
| **KU** | 내년 인건비 예산이 충분할지, 핵심 인력 이탈 리스크 |
| **UK** | 과거 의사결정 이력 데이터 (있지만 활용 안 함), 부서별 비용 패턴 |
| **UU** | 경쟁사의 복리후생 변화가 우리 채용에 미칠 영향, 규제 변경 리스크 |
...
자신의 경영지원 업무 전략에 대해 Known/Unknown 분석을 요청한다:
...
```
내 [경영회의 운영 / 인사기획 / 예산 관리] 전략에 대해 Known/Unknown 분석해줘
```
...
정답: Quiz 3-1은 2번. **plugin.json**이 플러그인의 핵심 파일이다. 이름, 버전, 설명, 키워드 등 메타데이터를 담고 있어 마켓플레이스에서 검색/설치의 기준이 된다.

정답: Quiz 3-2는 2번. clarify 플러그인에는 **3개 스킬** (vague, unknown, metamedium)이 포함되어 있다.

정답: Quiz 3-3은 2번. **vague는 모호한 요구사항을 구체적 스펙으로** 바꾸고 (Phase 1-4), **unknown은 전략의 사각지대를 4분면으로 분석**한다 (R1→R2→R3). 목적이 다르기 때문에 프로토콜도 다르다.
...
1. 경영지원 업무 중 가장 모호한 부분을 하나 고른다 (경영회의 준비, 인사 현황 보고, 예산 정리 등)
