...
Day 2에서 설치한 clarify 플러그인에는 `vague`라는 스킬이 있다. 모호한 요구사항을 구체적인 스펙으로 바꿔주는 스킬이다.
...
└───────────────────────────────────┘
                 ↓ Clarify (질문 6개)
┌─ After ───────────────────────────┐
│ 목표: 2분기 부서별 예산 집행 현황 보고│
│ 범위: 경영지원본부 5개 팀 대상       │
│ 형식: PDF, 10페이지 이내            │
│ 데이터: SAP 집행 내역 + 잔액        │
│ 제약: 이번 주 금요일 CFO 보고 전까지 │
│ 우선순위: 초과 집행 부서 > 정상 집행  │
└───────────────────────────────────┘
```

"예산 보고서 만들어줘"라는 5단어가 구체적인 실행 스펙으로 바뀌었다. 이 과정에서 사용자 스스로도 "아, 나는 전체가 아니라 경영지원본부만 필요했구나"를 발견하게 된다.
...
자신의 경영지원 업무 중 가장 모호한 부분을 하나 골라서, Claude에게 던져보자.

예시:

```
나는 [경영회의 자료 준비 / 인사 현황 정리 / 예산 보고서 작성]을 하고 싶어.
...
정답: Quiz 1-1은 3번. **Hypothesis-as-Options**이 clarify:vague의 핵심이다. 열린 질문이 아니라, Claude가 먼저 가설을 세워서 선택지로 제시한다. 체험 소감은 어떤 것이든 좋다 — 중요한 건 "clarify 받는 사람" 입장을 직접 경험한 것이다.
