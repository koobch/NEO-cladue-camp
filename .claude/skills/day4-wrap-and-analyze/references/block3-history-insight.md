...
- 위치: `~/.claude/projects/` (Windows: `%USERPROFILE%\.claude\projects\`) 폴더 안에 자동으로 저장됩니다
- 프로젝트별로 폴더가 나뉘어 있음
- 하나의 세션(대화) = 하나의 파일
...
|------|---------------------------|
| "이번 달 가장 많이 작업한 업무 유형은?" | 세션별 작업 내용을 분류하고 빈도순 정리 |
| "반복되는 패턴이 있나?" | 반복되는 보고, 재시도 패턴 추출 |
| "경영지원 업무별로 어떤 작업을 했는지 정리해줘" | 업무 유형별(재무/인사/총무) 세션 분류 및 요약 |
...
Block 2에서 사용했던 **작업 세션 창**에서 실습합니다. 창을 닫았다면 새 Git Bash 창 또는 Windows Terminal 탭(`Ctrl + Shift + T`)을 열고, 같은 프로젝트 폴더로 이동한 뒤 `claude`를 실행하세요. 교육 세션 창이 아닌 별도 창에서 진행합니다.
...
Claude가 `~/.claude/projects/` (Windows: `%USERPROFILE%\.claude\projects\`) 폴더 안의 세션 파일 목록을 보여줍니다. 프로젝트별로 폴더가 나뉘어 있고, 각 폴더 안에 세션 파일들이 있는 것을 확인하세요.
...
다른 질문 예시:
- "경영지원 업무 중 가장 자주 사용한 도구(Tool)는 뭐야?"
- "가장 긴 세션은 어떤 업무였어?"
- "이 패턴을 바탕으로 경영지원본부 워크플로우 개선 제안을 해줘"
...
| "jq가 필요합니다" | Windows에서는 jq 대신 Python을 사용합니다 (이미 설치됨). Claude에게 "jq 대신 Python으로 JSON을 파싱해줘"라고 요청하세요. 안 되면 강사에게 문의하세요. |
...
정답: 2번. 세션 파일의 대부분은 파일 변경 이력 같은 부속 자료다. 실제 사용자와 Claude의 대화는 약 6%만 차지한다. history-insight는 이 핵심 대화만 추출해서 분석하므로, 큰 파일도 빠르게 처리된다.
