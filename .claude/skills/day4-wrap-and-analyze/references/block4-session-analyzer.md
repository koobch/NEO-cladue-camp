...
Block 2에서 사용했던 작업 세션 창이 아직 열려 있으면 그 창에서, 닫았다면 새 Git Bash 창 또는 Windows Terminal 탭(`Ctrl + Shift + T`)을 열고 같은 프로젝트 폴더로 이동한 뒤 `claude`를 실행합니다.
