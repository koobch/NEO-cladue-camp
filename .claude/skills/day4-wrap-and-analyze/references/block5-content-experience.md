...
좋아하는 영어 트윗 URL을 하나 찾으세요. 경영, 조직 운영, HR, 재무 관련 트윗이면 더 좋습니다.
...
> 예: "session-wrap을 만들면서 multi-agent 패턴의 핵심이 '병렬 실행 + 중복 검증'이라는 걸 알게 됐다. 경영지원본부 주간 보고에도 이 패턴을 적용하면 재무/인사/총무 보고를 동시에 수집할 수 있겠다"

### (선택) 나만의 콘텐츠 파이프라인 구상

시간이 있다면, 앞으로 어떤 콘텐츠를 자동으로 가져와서 학습할지 생각해보세요:

- 매일 아침 경영/HR 인플루언서의 트윗을 가져와서 학습?
- 경영 전략, 재무 분석 관련 YouTube 영상을 자동으로 요약?
- 여러 소스를 모아서 경영지원본부 주간 다이제스트?
...
```
/team-assemble 이번 달 경영회의 안건을 분석하고, 개선점을 제안하고, 보고서 초안을 작성해줘
```
...
**정답 5-1: 3번.** 전체 번역부터 보면 핵심을 놓치기 쉽다. 요약으로 "이 트윗이 뭘 말하는지" 파악하고, 인사이트로 "나에게 어떤 의미인지" 정리한 뒤, 전체를 읽으면 맥락 속에서 이해도가 훨씬 높아진다.

**정답 5-2: 1번.** Pretesting Effect 연구에 따르면 학습 전 테스트가 기억력을 9-12% 향상시킨다. 퀴즈에서 틀리면 "왜 틀렸지?"라는 호기심(Information Gap)이 생기고, 이 상태에서 콘텐츠를 읽으면 기억이 더 잘 남는다.
